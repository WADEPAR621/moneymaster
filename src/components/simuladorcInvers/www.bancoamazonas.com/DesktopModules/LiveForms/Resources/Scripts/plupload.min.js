...
}(this);; (function (e) { var t = {}, n = e.moxie.core.utils.Basic.inArray; return function r(e) { var i, s; for (i in e) s = typeof e[i], s === "object" && ! ~n(i, ["Exceptions", "Env", "Mime"]) ? r(e[i]) : s === "function" && (t[i] = e[i]) }(e.moxie), t.Env = e.moxie.core.utils.Env, t.Mime = e.moxie.core.utils.Mime, t.Exceptions = e.moxie.core.Exceptions, e.mOxie = t, e.o || (e.o = t), t })(this);

/* Plupload * v2.1.2 * 2014-05-14*/
...
/*Helper functions*/
function mplhandleStatus(file) { var actionClass; if (file.status == plupload.DONE) { actionClass = 'plupload_done' } if (file.status == plupload.FAILED) { actionClass = 'plupload_failed' } if (file.status == plupload.QUEUED) { actionClass = 'plupload_delete' } if (file.status == plupload.UPLOADING) { actionClass = 'plupload_uploading' } var icon = $('#' + file.id).attr('class', actionClass).find('a').css('display', 'block'); if (file.hint) { icon.attr('title', file.hint) } } function mplupdateErrorFile(err) { var file = err.file, message; if (file) { message = 'code : ' + err.code + ',message: ' + err.message; if (err.details) { message += " (" + err.details + ")" } if (err.code == plupload.FILE_SIZE_ERROR) { alert(_("Error: File too large:") + " " + file.name) } if (err.code == plupload.FILE_EXTENSION_ERROR) { alert(_("Error: Invalid file extension:") + " " + file.name) } file.hint = message; $('#' + file.id).attr('class', 'plupload_failed').find('a').css('display', 'block').attr('title', message) } }